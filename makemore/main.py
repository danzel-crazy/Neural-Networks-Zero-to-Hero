...
class CharDataset(Dataset):
    ...
    def __init__(self, words, chars, max_word_length):
        ...
        self.stoi = {ch:id+1 for id, ch in enumerate(self.chars)}
        self.itos = {id:ch for ch, i in self.stoi.items()}
    ...
    def decode(self, ix):
        word = ''.join(self.itos(ix[i]) for i in ix)
        return word
...
def create_datasets(input_file):
    ...
    #10% of total names as test examples
    test_size = int(min(1000, len(words) * 0.1))
    indices = torch.randperm(len(words))
    
    train_data = [words[i] for i in indices[:-test_size]]
    test_data = [words[i] for i in indices[-test_size:]]
    print(f"split the dataset into {len(train_data)} training examples and {len(test_data)} testing examples" )
    
    train_dataset = CharDataset(train_data, chars, max_word_length)
    test_dataset = CharDataset(test_data, chars, max_word_length)
